...
static node* creer_noeud(position* pos){
...
static node* regle_arbre_aleatoire(node* tree){
...
static int renvoyer_hasard_coup_restant(node* n){
  // Entree : un noeud
...
static int regle_default(node* noeud, int joueur){
  // Entree : Un noeud
...
static void retropropagation(node* noeud, int joueur){
...
static int couleur_gris(node* n){
  // Entree : Un noeud
...
static void parcours_en_profondeur(node* racine, FILE* f, int* numero_noeud) {
...
static void affichage_arbre(node* arb){
  // Entree : Un arbre
...
int strategie_flat_mc(position* p){
  ...
  // printf("Le temps d'exécution de la strategie flat Monte-Carlo est de %f secondes\n", cpu_time_used);
